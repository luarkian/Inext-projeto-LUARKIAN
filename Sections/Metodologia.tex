...
\par
Na \autoref{fig:modelo}\todo[color=green]{Ampliar o texto da figura} é possível notar que há um dispositivo acoplado próximo a 
...
\begin{enumerate}
    ...
    \item Configuração de controle e monitoramento de transição dos objetos. \todo[color=green]{Sugiro mudar para Controle e monitoramento de transição dos objetos}
\end{enumerate}
...
\todo[color=green,inline]{Sugiro colocar uma foto de um objeto usando RFID}
\begin{figure}[H]
...
\todo[inline]{Sugiro descrever aquela ideia de dizer que além das transições entre as salas também é possivel identificar 
...
\par\todo{Sugiro descrever mais sobre este servidor web, exemplo de conexão local ou não, banco de dados e outras informações necessárias}
...
\todo[inline]{Sugiro descrever um pouco mais, talvez falar que com os dados de log das transições do sistema é possivel também delimitar
salas que objetos não podem sair, ou não podem entrar, criando zones de controle.}
...
conexão com uma rede weriless 802.11 b/g/n\todo{Falar um pouco sobre baixo consumo de energia}.
...
\todo[inline]{Sugiro descrever a função de cada um dos pinos acima.}
...
trabalhar em dois modos tanto como ponto de acesso ou no modo estação que envia e recebe dados\todo[color=green]{Descrever}. 
\begin{itemize}
    \item GND (\textit{graduated neutral density filter}) - conectado ao pino GND do Arduino;
...
\todo[inline]{Descrever a função dos pinos}
