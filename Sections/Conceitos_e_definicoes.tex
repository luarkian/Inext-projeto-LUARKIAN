...
Nesta seção serão abordados alguns dos algoritmos que já foram utilizados para localização em ambientes indoor.
    ...
    \par
    Os calculos são execultados da seguinte forma, havendo \textit{n} nós de referência, que serão repesentado por R_k, k = \left[ \textit{1, 2, ...,n} \right] com suas coordenadas ja conhecidas( x_k, y_k), T representando o nó alvo com coordenadas desconhecidas (x,y), podemos estimar utilizando a fórmula da distância da geometria analitica
     \Bigg\{ r_1^2 = (x - x_1 )^2 + (y - y_1)^2\\
